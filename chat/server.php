...
//     $hashCacheKey = $config['cache_key'];
    
//     foreach($server->connections as $fd){
//       // 加入redis判定，如果$fd存在redis中，则使用redis中存在的名字
//       $redis = new Redis();
//       $con_status= $redis->connect('118.190.22.125', 6380);
//       $redis->auth('redis_nihao123###');
//       $user_state = $redis->hmget($hashCacheKey,$fd,array('user_name'))['user_name'];
//       if ($user_state) {
//         // 有设置用户名
...
//         $server->push($fd , $fd . '加入了聊天');//返回用户利润表
        $server->push($fd , json_encode(["code"=>"1103","msg_info"=>$fd . '加入了聊天',"user_info"=> $request->fd]));//返回用户利润表
...
//     }
    foreach($server->connections as $fd){
        $server->push($fd , json_encode( ["code"=>"1103","msg_info"=>$frame->data,"user_info"=> $frame->fd]));//循环广播
    }
